...
describe("active Founder capacity migration", () => {
  it("counts only current active approved Founder subscriptions", () => {
    ...
    expect(counter).toContain("s.status = 'active'");
    expect(counter).toContain("s.founder_status = 'active'");
    expect(counter).toContain("s.founder_price_lock = true");
